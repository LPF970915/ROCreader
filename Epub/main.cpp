...
constexpr const char *kEpubImageMarkerPrefix = "[[EPUB_IMG:";
constexpr const char *kEpubImageMarkerSuffix = "]]";

bool TryParseEpubImageMarker(const std::string &line, std::string &out_key) {
  auto is_ideographic_space_utf8 = [&](size_t pos) -> bool {
    // U+3000 in UTF-8 is: E3 80 80
    return pos + 2 < line.size() &&
  ...
  size_t i = 0;
  while (i < line.size() &&
         (line[i] == ' ' || line[i] == '\t' || line[i] == '\r' || is_ideographic_space_utf8(i))) {
    if (is_ideographic_space_utf8(i)) i += 3;
    else ++i;
  }
  std::string trimmed = line.substr(i);
  while (!trimmed.empty()) {
    if (trimmed.back() == ' ' || trimmed.back() == '\t' || trimmed.back() == '\r') {
      trimmed.pop_back();
      continue;
    }
    // Trim trailing U+3000 (UTF-8: E3 80 80)
    if (trimmed.size() >= 3) {
      const size_t sz = trimmed.size();
      const unsigned char b0 = static_cast<unsigned char>(trimmed[sz - 3]);
      const unsigned char b1 = static_cast<unsigned char>(trimmed[sz - 2]);
      const unsigned char b2 = static_cast<unsigned char>(trimmed[sz - 1]);
  ...
  if (trimmed.size() <= std::strlen(kEpubImageMarkerPrefix) + std::strlen(kEpubImageMarkerSuffix)) return false;
  if (trimmed.rfind(kEpubImageMarkerPrefix, 0) != 0) return false;
  if (trimmed.size() < std::strlen(kEpubImageMarkerSuffix) ||
      trimmed.compare(trimmed.size() - std::strlen(kEpubImageMarkerSuffix),
                      std::strlen(kEpubImageMarkerSuffix),
                      kEpubImageMarkerSuffix) != 0) {
    return false;
  }
  const size_t start = std::strlen(kEpubImageMarkerPrefix);
  const size_t end = trimmed.size() - std::strlen(kEpubImageMarkerSuffix);
  if (end <= start) return false;
  out_key = trimmed.substr(start, end - start);
...
int main(int, char **) {
  ...
  std::cout << "[native_h700] epub backend: " << epub.BackendName()
            << " (available=" << (epub.Available() ? "yes" : "no") << ")\n";
  ...
  std::unordered_map<std::string, SDL_Texture *> epub_inline_images;
  std::filesystem::path epub_asset_cache_dir_runtime;
        ...
        for (const auto &root : books_roots) {
          ...
          for (std::filesystem::directory_iterator it(root_path, opts, ec), end; it != end; it.increment(ec)) {
            ...
            if (entry.is_regular_file(ec)) {
              ...
              const bool pure_txt = is_pure_ext_folder(entry.path().string(), ".txt");
              const bool pure_epub = is_pure_ext_folder(entry.path().string(), ".epub");
              if (!pure_txt && !pure_epub) continue;
              BookItem item;
              item.name = entry.path().filename().string();
              item.path = entry.path().string();
              item.is_dir = true;
  ...
  auto load_epub_inline_image = [&](const std::string &image_id) -> SDL_Texture * {
    auto it = epub_inline_images.find(image_id);
    if (it != epub_inline_images.end()) return it->second;
    if (epub_asset_cache_dir_runtime.empty() || image_id.empty()) {
      epub_inline_images[image_id] = nullptr;
      return nullptr;
    }
    std::string img_key = image_id;
    std::string img_ext = "bin";
    const size_t sep = image_id.find('|');
    if (sep != std::string::npos) {
      img_key = image_id.substr(0, sep);
      img_ext = image_id.substr(sep + 1);
      if (img_ext.empty()) img_ext = "bin";
    }
    const std::filesystem::path image_path = epub_asset_cache_dir_runtime / (img_key + "." + img_ext);
    SDL_Surface *surf = LoadSurfaceFromFile(image_path.string());
    if (!surf) {
      epub_inline_images[image_id] = nullptr;
      return nullptr;
    }
    SDL_Texture *tex = CreateTextureFromSurface(renderer, surf);
    SDL_FreeSurface(surf);
    epub_inline_images[image_id] = tex;
    if (tex) {
      int tw = 0;
      int th = 0;
      ...
      if (tw <= 0 || th <= 0) {
        ...
        remember_texture_size(tex, tw, th);
      }
    }
    return tex;
  };
  ...
  auto load_txt_cover = [&](const BookItem &item) -> SDL_Texture * {
    const std::string ext = GetLowerExt(item.path);
    const bool txt_file = (!item.is_dir && (ext == ".txt" || ext == ".epub"));
  ...
  auto focused_title_needs_marquee = [&]() -> bool { return false; };
#endif
...
#ifndef HAVE_SDL2_TTF
  auto persist_current_txt_resume_snapshot = [&](const std::string &, bool) {};
#endif
  ...
  auto open_epub_book = [&](const std::string &path) -> bool {
    EpubReader::ExtractedText extracted;
    std::string error;
    const std::filesystem::path epub_asset_cache_dir = txt_layout_cache_dir / "epub_assets";
    epub_asset_cache_dir_runtime = epub_asset_cache_dir;
    std::error_code tmp_ec;
    std::filesystem::create_directories(epub_asset_cache_dir_runtime, tmp_ec);
    // Cleanup inline image textures from previous epub book.
    for (auto &kv : epub_inline_images) {
      if (kv.second) SDL_DestroyTexture(kv.second);
    }
    epub_inline_images.clear();
    if (!epub.ExtractText(path, epub_asset_cache_dir.string(), extracted, error)) {
      std::cerr << "[reader] epub open failed: " << path << " reason=" << error << "\n";
      return false;
    }
    if (extracted.text.size() > kTxtMaxBytes) {
      extracted.text.resize(kTxtMaxBytes);
    }
    std::error_code ec;
    std::filesystem::create_directories(txt_layout_cache_dir, ec);
    const std::string cache_key = NormalizePathKey(path) + "|" +
                                  std::to_string(extracted.logical_size) + "|" +
                                  std::to_string(extracted.logical_mtime) + "|epub_img_v5";
    const size_t hash_value = std::hash<std::string>{}(cache_key);
    std::ostringstream oss;
    oss << std::hex << hash_value << ".epub.txt";
    const std::filesystem::path cache_path = txt_layout_cache_dir / oss.str();
    std::ofstream out(cache_path, std::ios::binary | std::ios::trunc);
    if (!out) {
      std::cerr << "[reader] epub cache write failed: " << cache_path.string() << "\n";
      return false;
    }
    out.write(extracted.text.data(), static_cast<std::streamsize>(extracted.text.size()));
    if (!out) {
      std::cerr << "[reader] epub cache write incomplete: " << cache_path.string() << "\n";
      return false;
    }
    return open_text_book(cache_path.string());
  };
      ...
      if (input.IsJustPressed(Button::L1)) {
        ...
        if (item.is_dir && current_folder.empty()) {
          ...
          if (ext == ".txt") {
            ...
            opened = open_epub_book(current_book);
          } else if (ext == ".pdf") {
    ...
    if (state == State::Boot) {
      ...
      if (state == State::Reader) {
        ...
        if (reader_mode == ReaderMode::Txt && txt_reader.open) {
          ...
          int i = start_line;
          while (i < static_cast<int>(txt_reader.lines.size())) {
            if (y > txt_reader.viewport_y + txt_reader.viewport_h) break;

            std::string image_key;
            if (TryParseEpubImageMarker(txt_reader.lines[i], image_key)) {
              // Render the image as its own block and skip drawing the lines
              // it covers to avoid overlaying text.
              bool drawn = false;
              if (SDL_Texture *img = load_epub_inline_image(image_key)) {
                int iw = 0;
                int ih = 0;
                get_texture_size(img, iw, ih);
                if (iw > 0 && ih > 0) {
                  const int max_w = std::max(1, txt_reader.viewport_w - 2);
                  const int max_h = std::max(1, txt_reader.viewport_h - 2);
                  // "Contain" mode: show all content (no cropping), still as large as possible.
                  const float scale = std::min(
                      static_cast<float>(max_w) / static_cast<float>(iw),
                      static_cast<float>(max_h) / static_cast<float>(ih));
                  const int dw = std::max(1, static_cast<int>(std::round(iw * scale)));
                  const int dh = std::max(1, static_cast<int>(std::round(ih * scale)));

                  SDL_Rect idst{
                      txt_reader.viewport_x + (txt_reader.viewport_w - dw) / 2,
                      y,
                      dw,
                      dh};
                  SDL_RenderCopy(renderer, img, nullptr, &idst);

                  // Skip enough lines so following text won't render underneath the image.
                  const int skip_lines = std::max(1, (dh + txt_reader.line_h - 1) / txt_reader.line_h);
                  i += skip_lines;
                  y += skip_lines * txt_reader.line_h;
                  drawn = true;
                }
              }
              if (!drawn) {
